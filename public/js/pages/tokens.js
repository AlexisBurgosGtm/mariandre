...
const tokensTable = 'w-full border-collapse text-left text-[11px] leading-snug';
const tokensTh =
...
const tokensTd = 'border-b border-slate-100 px-2 py-1.5 align-middle text-slate-600';
const tokensInput = cx(tw.input, 'max-w-xs px-2.5 py-1.5 text-xs');
const tokensPanel =
...
function renderTokensTable(tokens, selectedToken) {
  if (!tokens.length) {
    return `<p class="px-2 py-6 text-center text-xs text-slate-500">No hay tokens</p>`;
  }
  ...
  return `
    <table class="${tokensTable}">
      <thead>
        <tr>
          <th class="${tokensTh}">TOKEN</th>
          <th class="${tokensTh}">EMPRESA</th>
          <th class="${tokensTh}">ACTIVO</th>
      ...
      <tbody>
        ${tokens.map((t) => `
          <tr class="${cx(tokenRowBase, selectedToken === t.TOKEN && tokenRowSelected)} token-row" data-token="${escapeHtml(t.TOKEN)}">
            <td class="${tokensTd}"><code class="rounded bg-slate-100 px-1 py-0.5 font-mono text-[10px] text-slate-700">${escapeHtml(t.TOKEN)}</code></td>
            <td class="${tokensTd}">${escapeHtml(t.EMPRESA || '')}</td>
            <td class="${cx(tokensTd, 'activo-cell')}" data-token="${escapeHtml(t.TOKEN)}">${renderActivoBadge(t.ACTIVO)}</td>
            <td class="${cx(tokensTd, tw.tableActions)}">
              <button type="button" class="${cx(tw.btnGhost, 'px-2 py-1 text-[10px]')} btn-edit-token" data-token="${escapeHtml(t.TOKEN)}" title="Editar"><i class="fa-solid fa-pen"></i></button>
              <button type="button" class="${cx(tw.btnDanger, 'px-2 py-1 text-[10px]')} btn-delete-token" data-token="${escapeHtml(t.TOKEN)}" title="Eliminar"><i class="fa-solid fa-trash"></i></button>
            </td>
          </tr>
        `).join('')}
      </tbody>
...
function renderCommunityTable(rows) {
  ...
  if (!rows.length) {
    return `<p class="px-2 py-6 text-center text-xs text-slate-500">Sin registros para este TOKEN</p>`;
  }
  ...
  return `
    <table class="${tokensTable}">
      <thead>
        <tr>
          <th class="${tokensTh}">EMPNIT</th>
          <th class="${tokensTh}">Empresa</th>
          <th class="${tokensTh}">VPN</th>
          <th class="${tokensTh}">IP</th>
      ...
      <tbody>
        ${rows.map((r) => `
          <tr>
            <td class="${tokensTd}">${escapeHtml(r.EMPNIT || '')}</td>
            <td class="${tokensTd}">${escapeHtml(r.EMPNOMBRE || '')}</td>
            <td class="${tokensTd}">${escapeHtml(r.VPN_CODE || '')}</td>
            <td class="${tokensTd}">${escapeHtml(r.SERVER_IP || '')}</td>
      ...
      <div class="grid min-h-0 flex-1 grid-cols-1 grid-rows-2 gap-3 xl:grid-cols-2 xl:grid-rows-1">
        <section class="${tokensPanel}">
          <div class="mb-2 flex shrink-0 flex-wrap items-center justify-between gap-2">
            <h3 class="flex items-center gap-1.5 text-sm font-semibold text-slate-800"><i class="fa-solid fa-key text-xs"></i> TOKENS</h3>
            <input type="search" id="token-search" class="${tokensInput}" placeholder="Buscar token..." value="${escapeHtml(pageState.tokenSearch)}">
          </div>
        ...
        <section class="${tokensPanel}">
          <div class="mb-2 flex shrink-0 flex-wrap items-center justify-between gap-2">
            <h3 id="community-panel-title" class="min-w-0 truncate text-sm font-semibold text-slate-800">COMMUNITY_EMPRESAS_SYNC${pageState.selectedToken ? ` — ${escapeHtml(pageState.selectedToken)}` : ''}</h3>
            <div class="flex flex-wrap items-center gap-1.5">
              <input type="search" id="community-search" class="${tokensInput}" placeholder="Buscar..." value="${escapeHtml(pageState.communitySearch)}">
              <button type="button" class="${cx(tw.btnPrimary, 'px-2.5 py-1 text-[11px]')}" id="btn-add-community" ${pageState.selectedToken ? '' : 'disabled'}>
