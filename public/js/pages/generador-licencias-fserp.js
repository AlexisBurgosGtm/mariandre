...
/**
 * Licencias FS ERP (El Salvador) — catálogo y claves independientes de OnneB.
 * Misma lógica de tokens / TOKENS.LICENCIA que el generador OnneB.
 */
...
function getFormPayload(container, modulesRoot) {
  const tokenSel = container.querySelector('#lic-fserp-token');
  ...
  return {
    ...
    expiresAt: container.querySelector('#lic-fserp-expires')?.value || null,
    notes: container.querySelector('#lic-fserp-notes')?.value.trim() || '',
...
function applyCloudLicenseToForm(container, modulesRoot, data) {
  ...
  const expiresEl = container.querySelector('#lic-fserp-expires');
  if (expiresEl) expiresEl.value = expiresInputValue(data?.expiresAt || data?.license?.payload?.expiresAt);
  const notesEl = container.querySelector('#lic-fserp-notes');
...
export async function renderGeneradorLicenciasFserp(container) {
  showLoader(container, 'Cargando catálogo FS ERP y tokens…');
  ...
  const rootHint = catalog.productRoot
    ? `<code class="${tw.code}">${escapeHtml(catalog.productRoot)}</code>`
    : '—';
  ...
  container.innerHTML = `
    <div class="space-y-4">
      ...
      <div class="rounded-2xl border border-emerald-200 bg-emerald-50/80 px-4 py-3 text-sm text-emerald-900">
        <strong>FS ERP (El Salvador)</strong> — catálogo y claves RSA independientes de OnneB.
        Cada módulo nuevo de este producto aparece solo aquí (${catalog.moduleCount || 0} módulos · ${catalog.menuCount || 0} vistas).
        Origen: ${rootHint}
      </div>

      <form id="lic-fserp-form" class="space-y-4">
        <div class="grid grid-cols-1 gap-4 xl:grid-cols-2 xl:items-start">
          <div class="${tw.panel}">
            <h3 class="mb-3 text-base font-semibold text-slate-800">Licencia FS ERP</h3>
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-[minmax(0,1fr)_11rem] sm:items-end">
              <div class="${tw.formGroup}">
                <label class="${tw.label}" for="lic-fserp-token">Cliente / instalación (TOKEN)</label>
                <select class="${tw.input}" id="lic-fserp-token" required>
                  <option value="">— Seleccione token / instalación —</option>
                  ${tokenOptions || '<option value="" disabled>No hay tokens</option>'}
                </select>
              </div>
              <div class="${tw.formGroup}">
                <label class="${tw.label}" for="lic-fserp-expires">Vence (opcional)</label>
                <input class="${tw.input}" type="date" id="lic-fserp-expires">
              </div>
            </div>
            <div class="mt-3 flex flex-wrap gap-2">
              <button type="button" class="${tw.btnGhost}" id="lic-fserp-load-cloud" disabled title="Leer TOKENS.LICENCIA y marcar módulos/vistas">
                <i class="fa-solid fa-cloud-arrow-down"></i> Cargar licencia de nube
              </button>
            </div>
            <div class="${cx(tw.formGroup, 'mt-3')}">
              <label class="${tw.label}" for="lic-fserp-notes">Notas</label>
              <input class="${tw.input}" type="text" id="lic-fserp-notes" placeholder="Contrato, contacto, etc.">
            </div>
            <p class="mt-2 text-xs text-slate-500">
              Catálogo en vivo desde FS ERP (<code class="${tw.code}">MENU_GROUPS</code>).
              Use <strong>Cargar licencia de nube</strong> para recuperar los checks del token sin volver a marcarlos.
              Firmado con claves propias del producto (no interchangeables con OnneB).
            </p>
          </div>

          <div class="${tw.panel}">
            <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
              <h3 class="text-base font-semibold text-slate-800">Plantillas FS ERP</h3>
            </div>
            <div class="mb-3 flex flex-wrap items-end gap-2">
              <div class="min-w-[10rem] flex-1">
                <label class="${tw.label}" for="lic-fserp-tpl-name">Nombre</label>
                <input class="${tw.input}" type="text" id="lic-fserp-tpl-name" placeholder="Ej. POS básico SV…">
              </div>
              <button type="button" class="${tw.btnGhost}" id="lic-fserp-tpl-save">
                <i class="fa-solid fa-floppy-disk"></i> Guardar
              </button>
            </div>
            <div id="lic-fserp-templates" class="max-h-56 overflow-auto">${renderTemplatesList(templates)}</div>
          </div>
        </div>
        ...
        <div class="${tw.panel}">
          <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
            ...
            <div class="flex flex-wrap gap-2">
              <button type="button" class="${cx(tw.btnGhost, tw.btnSm)}" id="lic-fserp-reload" title="Recargar catálogo desde FS ERP">
                <i class="fa-solid fa-arrows-rotate"></i> Recargar
              </button>
              <button type="button" class="${cx(tw.btnGhost, tw.btnSm)}" id="lic-fserp-all">Marcar todos</button>
              <button type="button" class="${cx(tw.btnGhost, tw.btnSm)}" id="lic-fserp-none">Ninguno</button>
            </div>
          </div>
          <div id="lic-fserp-modules" class="${cx(tw.cardGrid, 'gap-3')}">${renderModules(catalog.modules)}</div>
        </div>

        <div class="flex flex-col gap-3">
          <div class="flex flex-wrap items-center gap-3">
            <button type="submit" class="${tw.btnPrimary}" id="lic-fserp-issue">
              <i class="fa-solid fa-download"></i> Generar y descargar
            </button>
            <button type="button" class="${tw.btnGhost}" id="lic-fserp-upload" disabled>
              <i class="fa-solid fa-cloud-arrow-up"></i> Generar y Subir licencia
            </button>
          </div>
          <p id="lic-fserp-status" class="text-sm text-slate-500" hidden></p>
        </div>
      </form>
      ...
      <aside class="${tw.panel}">
        ...
        <ol class="list-decimal space-y-1 pl-5 text-sm text-slate-600">
          ...
          <li><strong>Cargar licencia de nube</strong> marca las vistas ya subidas (útil tras agregar módulos en FS ERP).</li>
          ...
          <li>En FS ERP: <strong>Configuraciones → Licencia</strong> → descargar desde la nube y activar.</li>
        </ol>
      </aside>
    </div>
  `;

  const modulesRoot = container.querySelector('#lic-fserp-modules');
  const uploadBtn = container.querySelector('#lic-fserp-upload');
  const loadCloudBtn = container.querySelector('#lic-fserp-load-cloud');
  const tokenSel = container.querySelector('#lic-fserp-token');

  const setStatus = (msg, isErr = false) => {
    const el = container.querySelector('#lic-fserp-status');
    if (!el) return;
    el.hidden = !msg;
    el.textContent = msg || '';
  ...
  const loadLicenseFromCloud = async () => {
    ...
    setStatus('');
    try {
      const data = await api.getTokenLicenseFserp(token);
      ...
      const extra =
        unknown > 0
          ? ` · ${unknown} vista(s) de la licencia ya no están en el catálogo FS ERP`
          : '';
      setStatus(
  ...
  container.querySelector('#lic-fserp-reload')?.addEventListener('click', () => {
    renderGeneradorLicenciasFserp(container);
  });
  ...
  container.querySelector('#lic-fserp-all')?.addEventListener('click', () => {
  ...
  container.querySelector('#lic-fserp-none')?.addEventListener('click', () => {
    modulesRoot?.querySelectorAll('input[type=checkbox]').forEach((el) => {
      el.checked = false;
      el.indeterminate = false;
    });
  ...
  const templatesRoot = container.querySelector('#lic-fserp-templates');
  const refreshTemplates = async () => {
    try {
      templates = await api.getLicenseTemplatesFserp();
  ...
  container.querySelector('#lic-fserp-tpl-save')?.addEventListener('click', async () => {
    const name = String(container.querySelector('#lic-fserp-tpl-name')?.value || '').trim();
    ...
    try {
      await api.saveLicenseTemplateFserp({ name, menus });
      const nameInput = container.querySelector('#lic-fserp-tpl-name');
      if (nameInput) nameInput.value = '';
      await refreshTemplates();
      showToast('Plantilla FS ERP guardada', 'success');
    } catch (err) {
  ...
  templatesRoot?.addEventListener('click', async (e) => {
    ...
    if (delBtn) {
      ...
      try {
        await api.deleteLicenseTemplateFserp(id);
        await refreshTemplates();
  ...
  container.querySelector('#lic-fserp-form')?.addEventListener('submit', async (e) => {
    e.preventDefault();
    setStatus('');
    ...
    const btn = container.querySelector('#lic-fserp-issue');
    if (btn) btn.disabled = true;
    try {
      const data = await api.issueLicenseFserp({
      ...
      a.href = url;
      a.download = data.filename || `fserp-license-${payload.token}.json`;
      a.click();
      URL.revokeObjectURL(url);
      setStatus(
        `Licencia FS ERP generada: ${data.preview?.licenseId || ''} · ${payload.menus.length} vista(s) · ${(data.modules || []).length} módulo(s)`
      );
      showToast('Licencia FS ERP descargada', 'success');
    } catch (err) {
  ...
  uploadBtn?.addEventListener('click', async () => {
    ...
    uploadBtn.disabled = true;
    const issueBtn = container.querySelector('#lic-fserp-issue');
    ...
    try {
      const data = await api.issueAndUploadLicenseFserp({
        token: payload.token,
        expiresAt: payload.expiresAt,
        notes: payload.notes,
        menus: payload.menus,
      ...
      setStatus(
        `Licencia FS ERP subida a TOKENS (${data.token || payload.token}) · ${data.preview?.licenseId || ''} · ${payload.menus.length} vista(s)`
      );
      showToast('Licencia FS ERP generada y subida a la nube', 'success');
